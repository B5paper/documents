...
2. 配套代码 <https://github.com/Overv/VulkanTutorial/blob/main/code/03_physical_device_selection.cpp>
...
看起来 vulkan 通过枚举的方式给出可用的设备和设备支持的 command family，这一部分还是比较简单的。

logical device 是个新概念，不知道有啥用。windows surface，swap chain 都是新名词，猜不到有啥用。

任务列表：

1. [v] 学习 debug callback function P51~P55

1. [v] P55 ~ 57 debugging instance creation and destruction

    1. [v] 仔细推测代码中`setupDebugMessenger()`的作用

        这个函数和`pNext`做的工作应该差不多，唯一的不同是`setupDebugMessenger`只在 instance 创建完成后才起作用，而`pNext`会在 instance 创建的过程中就起作用。

    1. [v] [学习] 尝试显式创建 debug messenger

1. [v] 调研 Physical devices and queue families P58 ~ P65，给出任务列表

    看起来仍然是通过枚举的方式找到需要的设备。

    1. [ ] [学习] P58 ～ P61，尝试将 amd 的 gpu 枚举出来
...
1. 屏蔽 refraction 效果，先实现一个 path tracing
