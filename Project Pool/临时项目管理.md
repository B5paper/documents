...
* [v] opengl qa 完善或删除或缓存 `请使用 VBO 画一个 cube。`

    因为`[u_1]`目前还是 empty，所以不要让他出现在 qa 中。

* [ ] 把 opengl qa `请使用 shader 画一个绿色三角形。`的 u_1 换成
...
* [P] 调研`VkPipelineColorBlendStateCreateInfo`的作用 30 mins
...
* [v] vulkan 中`vkCmdBeginRenderPass`的参数`VK_SUBPASS_CONTENTS_INLINE`是什么意思？

* [v] 调研`VkPipelineInputAssemblyStateCreateInfo`的作用
...
* [v] 研究图 2-2，图 2-3 到图 2-4 的转换，尝试自己推导
