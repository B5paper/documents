...
* [v] 修改 opencl qa `请写一个向量相加的主程序。` 简洁版
...
* [ ] opengl qa 完善或删除或缓存 `请使用 VBO 画一个 cube。`
...
* [ ] 重新写一下 simple opencl。直接用 global 版本，`const char *`, `char *`都转换成`string`。
