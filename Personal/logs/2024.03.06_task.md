1. [v] cache tabs 10 mins

2. [v] 继续完成昨天的任务列表

    feedback:

    3. vulkan 画一个三角形的代码（2024.03.06）
        ...
        int main()
        {
            ...
            if (result != VK_SUCCESS)
            {
                printf("fail to create pipeline layout\n");
                exit(-1);
            }
            ...
            color_blend_attachment_state.colorWriteMask = VK_COLOR_COMPONENT_R_BIT |
                VK_COLOR_COMPONENT_G_BIT | VK_COLOR_COMPONENT_B_BIT | VK_COLOR_COMPONENT_A_BIT;  // 或许这行可以删掉？
...
3. [v] opencl: 调研 bug 如果一个 kernel 函数没有参数，那么调用 ocl simple 库就会出错

    15:50 ~ 16:40

4. [v] compiler: 整理编译器的笔记，并创建一个 qa

    17:24 ~ 17:51

5. [v] 整理上一道图论题，写一道新图论题
